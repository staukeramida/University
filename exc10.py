...
def rand_card():
    ...
    rand_value=random.choise(value_list)
    ...
    if tot_discarded-cards==0:
        ...
        while i< tot_discarded_cards:
            ...
            if not check_card_exists(discard_card,discard_card_list):
                continue
            discard_card_list.append(diascard_card)
            i=i+1
...
def identity_discard_cards(discard_list, player_hand):
    cards_to_remove=[]
...
def concatenate_player_values(ordered_hand):
    ...
    return [low_aces_values_str, high_aces_value_str]

#return true if sequential values is in the players hand else false
def is_straight():
    ordered_values_list=concatenate_player_value(ordered_hand)
...
def determine_hand(player_hand):
    processed_duplicate=compute_dup_values(player_hand)
...
def determine_high_card(player_hand):
    ...
    for value in reversed_valye_list:
        for card in player_hand:
            if card[0]=='A':
...
def opponent_hand(value_list):
    ...
    while i<2:
        ...
        if rand_value not in opponent_value_cards:
            opponent_values_cards.append(rand_value)
            i+=1
    if rand_num<=300:
        ...
        opp-hand='THREE OF A KIND(' +str(opponent_value_cards[0])+ 's)'
    elif rand_num<=975:
        opp_hand='STRAIGHT'
    elif rand_num<=987:
        opp_hand='FLUSH'
        ...
        opp_hand='FULL OF A KIND(' + str(opponent_value_cards[0])+ 's)'
    else:
...
#game loop
linebreak(0)
print('-'*15+ 'Anthony Starkey Python Poker'+'-'*15)
linebreak()
print('You play a british poker against a skilled player')
while True:
    time.sleep(3.5)
    ...
    ordered_hand=order_card_value(new_hand, value_list)
